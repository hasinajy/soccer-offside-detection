            ...
            try
            {
                if (_currentImagePath != null)
                {
                    ...
                    string outputPath = Path.Combine(
                        Path.GetDirectoryName(_currentImagePath) ?? string.Empty,
                ...
                {
                    MessageBox.Show("Unable to process image. Check if image is properly loaded.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            }
            catch (Exception ex)
            {
